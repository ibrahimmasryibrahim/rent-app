...
│   ├── Models/
│   │   ├── FileEntry.cs           ← ما قُرئ من القرص — لا يتغيّر أبدًا
│   │   ├── ReportRow.cs           ← الصف القابل للتعديل الذي يُطبع
│   │   └── ScanOptions, ReportOptions, ScanResult …
│   ├── Scanning/
...
│   ├── Printing/ReportPageRenderer.cs   ← رسم صفحات A4 للمعاينة والطباعة
│   ├── PrintPreviewWindow.xaml / .cs    ← معاينة الطباعة
│   ├── NumberPromptWindow.xaml / .cs    ← إدخال عدد صفوف مخصص
│   ├── Assets/app.ico                   ← أيقونة البرنامج
│   └── Infrastructure/  (RelayCommand, WindowsDialogService …)
│
└── tests/FileListPageCounter.Tests       (net8.0, xUnit — 40+ اختبارًا)
```

**فصل يضمن القاعدة الأساسية:** `FileEntry` هو ما قُرئ من القرص، و`ReportRow` هو ما يُطبع.
مولّدا Word و Excel لا يريان إلا `ReportRow`، فلا يوجد مسار كود واحد يمكن لتعديل الجدول أن
يصل عبره إلى ملف أصلي — الضمان يفرضه نظام الأنواع لا الانضباط.
...
## 4-د. المعاينة والتعديل والطباعة

الواجهة تبويبان: **الجدول** و**معاينة الطباعة**.

### الجدول — تعديل كامل قبل الحفظ
| العملية | كيف |
|---|---|
| تعديل اسم الملف | نقر مزدوج على الخلية |
| تعديل عدد الصفحات | نقر مزدوج — يقبل الأرقام العربية والإنجليزية، والفراغ يعني «غير معروف» |
| تعديل رقم الصف | نقر مزدوج على عمود «م» |
| إضافة صف | زر «صف جديد» — يُضاف بعد الصف المحدد |
| حذف صفوف | تحديد متعدد ثم «حذف المحدد» |
| إعادة الترتيب | «لأعلى» / «لأسفل» على التحديد |
| إعادة الترقيم | زر «إعادة الترقيم» يعيد العمود الأول 1، 2، 3 … |

### عدد الصفوف في الصفحة
قائمة مرنة: **تلقائي** (حسب ارتفاع الصفحة) أو 10 / 15 / 20 / 25 / 30 / 40 / 50 / 75 / 100
أو **عدد مخصص** يكتبه المستخدم (1 إلى 500). العدد ليس ثابتًا بأي حال.

### معاينة مباشرة
تبويب «معاينة الطباعة» يرسم صفحات A4 حقيقية بنفس تنسيق Word، وتتحدّث مع كل تعديل في الجدول.
**نفس المُحرِّك يرسم المعاينة ويطبع**، فلا يوجد تخطيطان يمكن أن يختلفا.

### الطباعة وخصائصها
زر «طباعة» يفتح **معاينة الطباعة** أولًا، وفيها لوحة خصائص:

- **اختيار الطابعة** من الطابعات المثبّتة (الافتراضية محدَّدة مسبقًا).
- **عدد النسخ**.
- **نطاق الصفحات** (من / إلى) — يُطبع فعليًا لا شكليًا.
- **خصائص الطابعة المتقدمة** — يفتح نافذة مشغّل الطابعة نفسها للوجهين والأدراج والجودة.

#### عناصر الصفحة — أخفِ ما لا تريده

في أعلى نافذة الطباعة صفٌّ من **أزرار الاختيار**، كل واحد يخفي عنصرًا من الصفحة، والمعاينة
تُرسم من جديد في اللحظة نفسها فترى الصفحة كما ستخرج من الطابعة تمامًا:

| الخيار | ما يخفيه |
|---|---|
| العنوان | العنوان الكبير في أول صفحة |
| تاريخ الإنشاء | سطر التاريخ تحت العنوان |
| ما تمت معالجته | شريط الإحصائيات أعلى الصفحة (عدد الملفات والصفحات وغير المعروف) |
| ترويسة أعلى كل صفحة | سطر العنوان الخفيف المتكرر أعلى الصفحات |
| الملخص في نهاية التقرير | كتلة الملخص بعد الجدول |
| أرقام الصفحات | «صفحة X» في التذييل |
| الاسم أسفل الصفحة | الاسم الصغير أسفل كل صفحة |

بإزالة العلامات كلها تُطبع **الصفحة والجدول وحدهما** — لا شيء غيرهما. وما تختاره هنا يُحفظ
في الجلسة ويسري كذلك على المعاينة الرئيسية وعلى ملفَّي Word و Excel، فلا يختلف مخرج عن آخر.

المخرج: A4 عمودي، RTL، رأس جدول متكرر في كل صفحة، هوامش 2 سم.

### لا شيء يُحفَظ إلا بطلب
`حفظ Word` و`حفظ Excel` و`طباعة` هي الأفعال الوحيدة التي تُنتج شيئًا. إغلاق البرنامج دون
الضغط عليها لا ينشئ أي ملف.

---

## 4-هـ. الاسم أسفل الصفحات

- الاسم `IBRAHIM MASRY IBRAHIM` **موجود افتراضيًا** أسفل كل صفحة، ويمكن تغييره إلى أي اسم آخر
  من المربع أعلى الجدول، أو مسحه فلا يظهر شيء.
- يظهر **مجرّدًا**: الاسم وحده بحجم **9 نقاط**، بلا أي كلمة قبله — لا «إعداد» ولا «مطور»
  ولا Developer / Developed by / Created by / Programmer. يوجد اختبار يفشل إن ظهرت أي منها.
- يمكن إخفاؤه من خيار «إظهاره في التقرير والطباعة» أو من نافذة الطباعة، دون مسح الاسم.
- الاسم يُطبع حتى لو أُلغيت أرقام الصفحات — لكل منهما زر اختيار مستقل.

> **متن التقرير لا يحمل اسم أي شخص**، ولا خصائص الملف: حقل «الكاتب» في خصائص المستند يحمل
> اسم الأداة نفسها (`FILE LIST & PAGE COUNTER`) لا اسم شخص. الاسم يظهر في موضع واحد فقط:
> السطر الصغير أسفل الصفحة.
...
الخيارات تعيش في الجلسة فقط، ويبدأ البرنامج من هذه القيم في كل تشغيل:
...
| حجم الخط | 20 |
| عدد الصفوف في الصفحة | تلقائي (حسب ارتفاع الصفحة) |
| عناصر الصفحة (العنوان، التاريخ، الإحصائيات، الترويسة، الملخص، أرقام الصفحات) | كلها ظاهرة |
| الاسم أسفل الصفحة | `IBRAHIM MASRY IBRAHIM`، ظاهر |
| الترتيب | حسب اسم الملف (Natural Sort) |

**لا يُكتب أي ملف إعدادات على القرص** — لا في `%APPDATA%` ولا بجوار الـ EXE ولا (بالطبع)
داخل مجلد المصدر. البرنامج محمول بالكامل: نسخه ونقله لا يترك أثرًا خلفه.
