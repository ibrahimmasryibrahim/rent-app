...
**للقراءة فقط**، تحسب عدد صفحات كل ملف، ثم تنتج تقريرًا احترافيًا جاهزًا للطباعة
بصيغة **Microsoft Word (DOCX)** أو **Microsoft Excel (XLSX)**.

**تطوير: Ibrahim Masry Ibrahim**
...
│   ├── Reporting/
│   │   ├── ReportLayout.cs        ← توزيع الجدول على الصفحة وتقدير عدد الصفحات
│   │   ├── ReportTheme.cs         ← لوحة الألوان وسلّم الأحجام المشتركة
│   │   ├── WordReportBuilder.cs   ← Open XML SDK: إنشاء DOCX حقيقي
│   │   └── ExcelReportBuilder.cs  ← Open XML SDK: إنشاء XLSX حقيقي
...
│   ├── ExportOptionsWindow.xaml / .cs   ← نافذة الخيارات قبل التصدير
│   └── Infrastructure/  (RelayCommand, WindowsDialogService …)
│
...
| إنشاء ملفات مؤقتة/Cache/Backup في مجلد المصدر | البرنامج **لا ينشئ ملفات مؤقتة أصلًا**، ولا يحفظ أي ملف إعدادات في أي مكان؛ سجل التفاصيل وحده يُحفظ في `%LOCALAPPDATA%` وبطلب صريح من المستخدم |
...
- **العنوان** قابل للتغيير — Bold، في المنتصف، ثم إجمالي عدد الملفات وإجمالي عدد الصفحات.
...
- **اسم المطوّر** في نهاية التقرير وفي خصائص المستند (Creator).

---

## 4-ب. مخرجات Excel

نفس البيانات في **XLSX حقيقي** بصيغة Open XML (وليس CSV بامتداد مُغيَّر)، مبني بنفس
مكتبة Open XML SDK — بلا أي مكتبة إضافية:

- **ورقة RTL** يظهر فيها عمود «م» على اليمين.
- **تجميد رأس الجدول** فيبقى ظاهرًا أثناء التمرير، و**تكراره في كل صفحة عند الطباعة**
  (`_xlnm.Print_Titles`).
- **فلتر تلقائي** (AutoFilter) على الأعمدة الثلاثة للفرز والبحث.
- **عدد الصفحات كأرقام حقيقية** لا كنص، فيمكن جمعها ورسمها بيانيًا. الملفات التي تعذر
  تحديد صفحاتها وحدها تُكتب «غير معروف» حتى لا تُفسد الجمع.
- **صف إجمالي** في نهاية الجدول.
- **A4 Portrait** مع ضبط العرض على صفحة واحدة عند الطباعة.
- **اسم المطوّر** في نهاية الورقة وفي خصائص الملف.

---

## 4-ج. عنوان التقرير

- **مربع نص** في الواجهة يكتب فيه المستخدم أي عنوان يريده.
- **خيار «استخدام اسم المجلد كعنوان للتقرير»** (مفعّل افتراضيًا): عند اختيار مجلد يصبح
  العنوان تلقائيًا هو اسم المجلد نفسه.
- العنوان يُقترح أيضًا **اسمًا للملف** في نافذة الحفظ، بعد إزالة الرموز التي يرفضها Windows.
- عنوان فارغ يعود تلقائيًا إلى «قائمة الملفات وعدد الصفحات».

---

## 4-د. نافذة الخيارات قبل التصدير

الضغط على «تصدير إلى Word» أو «تصدير إلى Excel» يفتح نافذة تعرض ما سيُنتَج **قبل** اختيار
مكان الحفظ:

- **عنوان التقرير** قابل للتعديل.
- **توزيع الجدول على الصفحة** (لملف Word): عمود واحد / عمودان / ثلاثة أعمدة — تتكرر
  الأعمدة الثلاثة عرضيًا فتستغل عرض A4 كاملًا.
- **حجم الخط** (16 / 18 / 20 / 22 / 24)، مع تنبيه يقترح حجمًا أصغر مع الأعمدة المتعددة.
- **عدد الصفحات المتوقّع**، يتحدّث فورًا مع كل اختيار ويوضّح كم صفحة يوفّرها التوزيع
  مقارنةً بعمود واحد.
- **فتح الملف بعد إنشائه**.

الترتيب داخل الأعمدة **رأسي**: تنزل الأرقام في العمود الأول حتى نهايته ثم تبدأ في الثاني،
كما تُقرأ أعمدة الجريدة، فيبقى الترقيم متسلسلًا للعين.

> **لا يُحفظ أي إعداد على القرص.** البرنامج يبدأ من القيم الافتراضية في كل تشغيل، ولا
> يترك أي ملف إعدادات في `%APPDATA%` ولا بجوار الـ EXE.
...
البرنامج لا يكتب أي ملف إعدادات في أي مكان — يبدأ من القيم الافتراضية في كل تشغيل.
