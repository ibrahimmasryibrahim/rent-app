...
public class ExcelReportTests
{
    ...
    public async Task The_sheet_is_right_to_left_with_a_frozen_header()
    {
        ...
        Assert.Equal(7D, pane.VerticalSplit!.Value); // freeze everything above the data
    }
    ...
    public async Task The_header_carries_the_three_required_columns()
    {
        ...
        string[] header = RowAt(data, 7).Elements<Cell>().Select(CellText).ToArray();
    ...
    public async Task Each_file_becomes_a_row_with_its_name_and_page_count()
    {
        ...
        Assert.Equal(new[] { "1", "10001", "4" }, RowAt(data, 8).Elements<Cell>().Select(CellText).ToArray());
        Assert.Equal(new[] { "2", "10002", "7" }, RowAt(data, 9).Elements<Cell>().Select(CellText).ToArray());

        // The damaged file is listed rather than dropped, exactly as in the Word report.
        Assert.Equal(new[] { "3", "10004", Strings.Unknown }, RowAt(data, 10).Elements<Cell>().Select(CellText).ToArray());
        Assert.Equal(new[] { "4", "وثيقة 10003", "1" }, RowAt(data, 11).Elements<Cell>().Select(CellText).ToArray());
    }
    ...
    public async Task Page_counts_are_stored_as_numbers_so_Excel_can_sum_them()
    {
        ...
        Cell knownCount = RowAt(data, 8).Elements<Cell>().Single(c => c.CellReference == "C8");
        Assert.Equal(CellValues.Number, knownCount.DataType!.Value);

        // Only the unknown one falls back to text.
        Cell unknownCount = RowAt(data, 10).Elements<Cell>().Single(c => c.CellReference == "C10");
    ...
    public async Task The_totals_and_the_grand_total_row_are_correct()
    {
        ...
        // The headline figures: labels on row 4, the numbers themselves on row 5.
        Assert.Equal(
            new[] { Strings.TotalFiles, Strings.TotalPages, Strings.UnknownFiles },
            RowAt(data, 4).Elements<Cell>().Select(CellText).ToArray());

        // 4 files, 4 + 7 + 1 pages (the damaged file counts nothing), 1 undetermined.
        Assert.Equal(new[] { "4", "12", "1" }, RowAt(data, 5).Elements<Cell>().Select(CellText).ToArray());

        Row grandTotal = RowAt(data, 12);
        Assert.Equal(Strings.GrandTotal, CellText(grandTotal.Elements<Cell>().Single(c => c.CellReference == "B12")));
        Assert.Equal("12", CellText(grandTotal.Elements<Cell>().Single(c => c.CellReference == "C12")));
    }
    ...
    public async Task The_header_row_repeats_on_every_printed_page_and_the_page_is_A4()
    {
        ...
        Assert.Contains("$7:$7", printTitles.Text);
    ...
    public void An_empty_result_still_produces_a_readable_workbook()
    {
        ...
        Assert.Equal(3, RowAt(data, 7).Elements<Cell>().Count()); // the header is still there
    }
    ...
    public async Task A_large_workbook_holds_every_row()
    {
        ...
        Assert.Equal(count, data.Elements<Row>().Count(r => r.RowIndex!.Value >= 8 && r.RowIndex.Value <= 7 + count));
        Assert.Equal("1000", CellText(RowAt(data, (uint)(8 + count)).Elements<Cell>().Single(c => c.CellReference == "C" + (8 + count))));
    }
}
